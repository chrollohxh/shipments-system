...
(() => {
  ...
  const operationQr = (record, position) => {
    ...
    try {
      ...
      code.addData(target);
      code.make();
      const pos = Object.assign({ xPercent: 85, yPercent: 20, widthPercent: 9, rotate: 0 }, position || {});
      return `<a class="operation-qr" href="${safe(target)}" target="_blank" rel="noopener" title="فتح ملف العملية ${safe(record.operationNo || '')}" style="left:${Number(pos.xPercent) || 0}%;top:${Number(pos.yPercent) || 0}%;width:${Number(pos.widthPercent) || 9}%;transform:rotate(${Number(pos.rotate) || 0}deg)"><img src="${code.createDataURL(3, 0)}" alt="QR: ${safe(record.operationNo || 'ملف العملية')}"></a>`;
    } catch (error) {
  ...
  const baharSheet = (record, kind, sourceCompany = companyForShipment(record)) => {
    ...
    const qrPos = Object.assign({ xPercent: 85, yPercent: 20, widthPercent: 9, rotate: 0 }, settings.qrPosition || {});
    const qr = isPack ? '' : operationQr(record, qrPos);
    ...
    const body = `
      ...
      <style>.bahar-shipment-sheet .bs-invoice-title{border-bottom:1px solid var(--bs-section-border-color, #000)!important}</style>
